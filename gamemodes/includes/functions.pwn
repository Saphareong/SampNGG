...
ShowCasinoGamesMenu(playerid, dialogid)
{
	switch(dialogid)
	{
		...
		{
			return ShowPlayerDialog(playerid, DIALOG_CGAMESCREDITS, DIALOG_STYLE_MSGBOX, "{FFFFFF}Casino Games - (Creadits)", "{FFFFFF}Chinh sua boi: GvC GTA-SA.VN", "Quay Lai", "");
		}
...
public InitiateGamemode()
{
	...
	print("Copyright (C) GvC Group, LLC (2015-2100)");
...
public Maintenance()
{
	...
	SendRconCommand("hostname GvC-Cong dong GTA online Viet Nam [Khoi dong lai de Bao tri]");
	foreach(new i: Player)
	{
		if(gPlayerLogged{i}) {
...
public SyncTime()
{
	...
	if ((tmphour > ghour) || (tmphour == 0 && ghour == 23))
	{
		...
		foreach(new i: Player)
		{
			if(PlayerInfo[i][pLevel] <= 5) SendClientMessageEx(i,COLOR_LIGHTBLUE,"Viec su dung hack/cleo tai may chu GvC ban se bi xoa tai khoan vinh vien khoi he thong!");
